	...
	%>
	    ...
	    <section id="pageList">
			<%if(nowPage<=1){ %>
				[이전]&nbsp;&nbsp;
			<%}else{ %>
				<a href="snsFollowLoginAction.sns?page=<%=nowPage-1%>&cust_id=<%=id%>">[이전]</a>&nbsp;
			<%} %>
			...
			<%for(int a=startPage;a<=endPage;a++){
				...
				[<%=a%>]&nbsp;&nbsp;
			<%}else{ %>
				<a href="snsFollowLoginAction.sns?page=<%=a%>&cust_id=<%=id%>">[<%=a%>]
				</a>&nbsp;
			<%} %>
			<%} %>
			<%if(nowPage>=maxPage){ %>
				[다음]
			<%}else{ %>
				<a href="snsFollowLoginAction.sns?page=<%=nowPage+1%>&cust_id=<%=id%>">[다음]</a>
			<%} %>
		</section>
			<%
			}
